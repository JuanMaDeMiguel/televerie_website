...
\newpage
\section*{Page des Réservations}

La page \textbf{Réservations} constitue l'espace central de planification de l'application. Alors que la page d'accueil permet surtout de consulter rapidement l'état global de la laverie, cette vue donne à l'étudiant les outils nécessaires pour \textbf{anticiper sa lessive}, \textbf{choisir un créneau pertinent} et \textbf{retrouver l'historique détaillé de ses sessions précédentes}. 

Cette interface a été pensée comme une expérience mobile complète : l'utilisateur peut y voir son prochain tour, consulter un aperçu du calendrier du mois, identifier les jours de plus forte ou de plus faible demande, puis ouvrir une fenêtre modale pour choisir précisément une date et une heure. L'ensemble garde ainsi une logique simple : \textit{observer, décider, réserver}.

\subsection*{Vue d'ensemble de l'écran}
L'écran est organisé en quatre cartes principales superposées verticalement :
\begin{itemize}
    \item une \textbf{carte de synthèse} affichant la prochaine réservation de l'utilisateur sous forme de compte à rebours ;
    \item une \textbf{carte calendrier} offrant une vue rapide du mois courant avec une légende visuelle ;
    \item une \textbf{carte statistique} représentant la demande sur les sept prochains jours ;
    \item une \textbf{carte d'historique} listant les dernières sessions avec leur durée, leur prix, leur statut et leur mode de paiement.
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.31\textwidth}
        \centering
        \includegraphics[width=\linewidth]{reservations_overview.png}
        \caption{Partie supérieure de l'écran}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.31\textwidth}
        \centering
        \includegraphics[width=\linewidth]{reservations_overview_2.png}
        \caption{Partie inférieure de l'écran}
    \end{subfigure}
    \hfill
    \hfill
    \caption{Vue d'ensemble de l'onglet Réservations avec la synthèse, le calendrier, la demande journalière et l'historique.}
    \label{fig:reservations-overview}
\end{figure}

\subsection*{La synthèse de la prochaine réservation}
La première carte agit comme un rappel immédiat. Si une réservation est déjà active, l'utilisateur voit apparaître un \textbf{compte à rebours en heures et minutes} avant son prochain créneau, accompagné d'un résumé textuel comprenant le jour, l'heure et la machine concernée. Cette présentation répond à un besoin pratique : éviter d'oublier un créneau déjà réservé et réduire le stress lié à l'organisation du lavage.
...
\subsection*{Le calendrier d'aperçu et la logique de recommandation}
La seconde carte présente un \textbf{aperçu mensuel du calendrier}. Chaque jour est affiché sous forme de bouton compact. Une légende simple permet de comprendre la logique visuelle :
\begin{itemize}
    \item \textbf{Bleu} : créneau recommandé ;
    \item \textbf{Rouge} : aucun créneau disponible ;
    \item \textbf{Gris clair} : jour passé ou non sélectionnable.
\end{itemize}

Ce choix graphique répond à un objectif ergonomique important : permettre à l'utilisateur de \textbf{repérer d'un coup d'œil les meilleures opportunités de réservation} sans devoir ouvrir immédiatement le sélecteur détaillé. La recommandation n'est pas arbitraire : elle repose sur une estimation de la demande, et le système met en avant le jour futur le moins chargé parmi ceux qui restent disponibles.

\subsection*{La demande par jour}
Sous le calendrier, une carte supplémentaire affiche la \textbf{demande journalière sur sept jours}. La représentation choisie est un histogramme vertical très simple, adapté à un écran mobile. Chaque colonne correspond à un jour et la barre la plus haute est mise en valeur en bleu afin d'indiquer le pic de demande.
...
\subsection*{L'historique des réservations}
La dernière carte contient l'\textbf{historique des 4 dernier réservations  passées}. Chaque ligne affiche la machine utilisée, la durée de la session, la date, le prix payé ainsi que le statut final de la réservation. Une icône différente est affichée selon le mode de paiement utilisé, ce qui rend la lecture plus visuelle et facilite la distinction entre carte bancaire, espèces ou QR code.
...
\subsection*{Réserver un nouveau créneau}
Lorsque l'utilisateur touche la carte du calendrier, l'application ouvre une \textbf{fenêtre modale de réservation}. Cette fenêtre est structurée en deux étapes successives :
\begin{enumerate}
    \item choisir une date ;
    \item choisir une heure.
\end{enumerate}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.31\textwidth}
        \centering
        \includegraphics[width=\linewidth]{reservations_reserve_date.png}
        \caption{Choix initial de la date}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.31\textwidth}
        \centering
        \includegraphics[width=\linewidth]{reservations_reserve_date_2.png}
        \caption{Créneau horaire sélectionné}
    \end{subfigure}
    \hfill
    \hfill
    \caption{Fenêtre modale de réservation permettant de choisir un jour puis une heure.}
    \label{fig:reservations-booking}
\end{figure}

\subsection*{Consulter et annuler sa réservation}
La carte de synthèse située en haut de l'écran est également interactive. En la touchant, l'utilisateur ouvre une seconde \textbf{fenêtre modale de détail}. Cette modale a deux états possibles :
\begin{itemize}
    \item un état \textbf{informatif} lorsqu'une réservation est active ;
    \item un état \textbf{vide} lorsqu'aucune réservation n'est programmée.
\end{itemize}

Dans le premier cas, l'application affiche le nom de la machine, la date, l'heure, la durée et le prix. Un bouton rouge permet alors d'\textbf{annuler la réservation}. Dans le second cas, la modale joue surtout un rôle de confirmation visuelle : elle informe clairement l'utilisateur qu'aucun prochain tour n'est réservé. Cette distinction entre état plein et état vide améliore la lisibilité générale du système.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.31\textwidth}
        ...
        \caption{Réservation active}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.31\textwidth}
        \centering
        \includegraphics[width=\linewidth]{reservations_your_reservation_No_reservation.png}
        \caption{Aucune réservation en cours}
    \end{subfigure}
    \hfill
    \hfill
    \caption{Fenêtre modale de détail de réservation selon les deux états possibles de l'application.}
    \label{fig:reservations-detail}
\end{figure}
...
\begin{itemize}
    \item \texttt{reservations.js}, chargé du rendu de l'interface et des interactions utilisateur ;
    \item \texttt{reservations.service.js}, chargé de gérer les données, la persistance locale et les opérations de réservation/annulation.
\end{itemize}
...
\subsection*{Une vue pilotée par un état centralisé}
Le fichier \texttt{reservations.js} repose sur un objet \texttt{state} contenant toutes les informations nécessaires au fonctionnement de l'écran : le mois affiché, la date sélectionnée, l'heure sélectionnée, les créneaux disponibles et la modale active.
...
\subsection*{Recommandation automatique d'une date}
...
\subsection*{Un système de réservation cohérent et persistant}
Le service \texttt{reservations.service.js} joue le rôle d'une petite base de données locale. Il charge d'abord les données depuis le fichier \texttt{reservations.json}, vérifie leur validité, puis les conserve dans le navigateur via \texttt{localStorage}. Ainsi, lorsqu'un utilisateur change d'onglet ou recharge la vue, l'état de sa réservation reste cohérent.

Lorsqu'un créneau est confirmé, le service effectue plusieurs opérations successives :
\begin{itemize}
    \item vérifier que la date et l'heure sont valides ;
    \item refuser les créneaux déjà passés ou déjà réservés ;
    \item retirer l'ancienne réservation éventuelle ;
    \item enregistrer le nouveau créneau dans la liste des slots occupés ;
    \item mettre à jour la demande journalière ;
    \item sauvegarder l'ensemble en mémoire locale.
\end{itemize}

\newpage

\begin{lstlisting}[caption={Principe simplifie de validation et de sauvegarde d'une reservation}]
async function reserveSlot(dateKey, time) {
    if (!isFutureSlot(dateKey, time)) {
        return { ok: false, message: "Impossible de reserver un creneau passe." };
    }

    const daySlots = dbCache.bookedSlots[dateKey] || [];
    if (daySlots.includes(time)) {
        return { ok: false, message: "Ce creneau est deja pris." };
    }

    if (dbCache.nextReservation) {
        updateDemand(dbCache.nextReservation.date, -1);
    }

    dbCache.nextReservation = { date: dateKey, time, ... };
...
\subsection*{Une interaction pensée pour le mobile}
Au-delà du simple affichage, la vue a été conçue avec plusieurs détails d'interaction :
\begin{itemize}
    \item ouverture et fermeture de modales sans rechargement de page ;
    \item fermeture au clic sur l'arrière-plan ;
    \item navigation clavier avec \texttt{Escape}, \texttt{ArrowLeft} et \texttt{ArrowRight} ;
    \item changement de mois par glissement horizontal ;
    \item adaptation responsive pour petits écrans.
\end{itemize}

Le fichier \texttt{reservations.css} complète cette logique avec une présentation très proche d'une application mobile native : fond flou derrière les modales, cartes arrondies, boutons larges et zones tactiles confortables. L'ensemble donne une sensation de fluidité cohérente avec le reste de l'application.

\subsection*{Intégration avec le reste de l'application}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{navigation.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{profil_principal.png}
